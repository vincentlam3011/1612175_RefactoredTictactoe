...
function ticTacToeApp(state = initState, action) {
  switch (action.type) {
    ...
    case GO_TO_MOVE: {
      if (action.step !== state.history.length) gameOver = false;
      return { ...state, history: state.history.slice(0, action.step + 1),
        stepNum: action.step,
    ...
    case CLICK_SQUARE: {
      ...
      return { ...state, // history: [...state.history, { squares: squares, latestMoveSqr: action.i }],
        history: history.concat([
          { squares, latestMoveSqr: action.i }
        ]),
